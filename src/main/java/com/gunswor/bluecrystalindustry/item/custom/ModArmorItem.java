package com.gunswor.bluecrystalindustry.item.custom;

import com.google.common.collect.ImmutableMap;
import com.gunswor.bluecrystalindustry.item.ModArmorMaterials;
...
public class ModArmorItem extends ArmorItem {
    private static final Map<ArmorMaterial, List<StatusEffectInstance>> MAP =
            (new ImmutableMap.Builder<ArmorMaterial, List<StatusEffectInstance>>())
                    .put(ModArmorMaterials.AZURITE.value(),
                            Arrays.asList(new  StatusEffectInstance(StatusEffects.FIRE_RESISTANCE, 40, 1, false, false, true),
                                    new StatusEffectInstance(StatusEffects.REGENERATION, 40, 1, false, false, true),
                                    new StatusEffectInstance(StatusEffects.NIGHT_VISION, 40, 1, false, false, true))).build();
        ...
        for (Map.Entry<ArmorMaterial, List<StatusEffectInstance>> entry : MAP.entrySet()) {
            ...
            if (hasCorrectArmorSet(player, material)) {
                for (StatusEffectInstance effect : effects) {
                    addStarusEffectForMaterial(player, effect);
                }
            }
        }
    }

    private void addStarusEffectForMaterial(PlayerEntity player, StatusEffectInstance effect) {
